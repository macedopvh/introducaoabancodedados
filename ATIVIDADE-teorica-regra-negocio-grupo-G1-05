...
**Data:** 18/08/2026  
**Repositório Git:** https://github.com/macedopvh/introducaoabancodedados
...
Triggers permitem executar funções automaticamente quando determinados eventos acontecem, como `INSERT`, `UPDATE` ou `DELETE`. Podem ser utilizadas para registrar alterações ou executar determinadas ações automaticamente.
...
O grupo entende que **não existe uma opção vencedora absoluta**. A melhor solução depende da natureza da regra.

### Sistema acessado por múltiplas aplicações

Quando diferentes sistemas acessam o mesmo Banco de Dados, regras fundamentais de integridade devem ser protegidas no próprio Banco de Dados. Por exemplo, a regra “CPF deve ser único” não deve depender somente da validação de uma aplicação.

### Dados sensíveis ou com exigência legal/fiscal

Quanto maior o impacto de uma inconsistência, maior a necessidade de mecanismos que garantam a integridade independentemente da aplicação. Nesse cenário, regras fundamentais devem possuir proteção no Banco de Dados, além das validações e controles existentes na aplicação.
...
O Banco de Dados garante identificação dos registros, campos obrigatórios, CPF único, valor positivo do serviço e referências para clientes e serviços existentes.
...
A **Tay Podologia e Estética** é utilizada como exemplo real para aplicação dos conceitos. Em um sistema de atendimento da empresa, poderiam existir clientes, profissionais, serviços, agendamentos, atendimentos e pagamentos.
...
> **Nota:** O caso da Tay Podologia e Estética é utilizado exclusivamente como exemplo prático para relacionar os conceitos teóricos da atividade a um cenário empresarial real. As regras apresentadas são exemplos acadêmicos e não representam necessariamente requisitos atualmente implementados pela empresa.
...
Se uma regra existir somente na aplicação, outro sistema poderá acessar o Banco de Dados sem passar por essa validação. Por outro lado, colocar todas as regras somente no Banco de Dados pode gerar problemas quando regras complexas de processo forem transformadas em triggers e procedures difíceis de manter.
...
Por isso, deve existir uma definição clara de responsabilidade. A aplicação pode **validar e orientar** o usuário, enquanto o Banco de Dados deve **garantir as invariantes fundamentais dos dados**.

---

# 3. Referências

- POSTGRESQL GLOBAL DEVELOPMENT GROUP. **PostgreSQL Documentation — Constraints**. Disponível em: https://www.postgresql.org/docs/current/ddl-constraints.html
- POSTGRESQL GLOBAL DEVELOPMENT GROUP. **PostgreSQL Documentation — CREATE TRIGGER**. Disponível em: https://www.postgresql.org/docs/current/sql-createtrigger.html
- POSTGRESQL GLOBAL DEVELOPMENT GROUP. **PostgreSQL Documentation — User-Defined Procedures**. Disponível em: https://www.postgresql.org/docs/current/xproc.html
- POSTGRESQL GLOBAL DEVELOPMENT GROUP. **PostgreSQL Documentation — Transaction Isolation**. Disponível em: https://www.postgresql.org/docs/current/transaction-iso.html
...
A atividade demonstrou que regras de negócio podem ser implementadas tanto no Banco de Dados quanto na aplicação, mas cada camada possui responsabilidades diferentes.

O Banco de Dados é especialmente importante para garantir regras fundamentais de integridade, como obrigatoriedade, unicidade, integridade referencial, valores válidos e consistência durante transações. A aplicação é mais adequada para fluxos, comportamentos, cálculos, decisões, regras complexas e interação com o usuário.

O grupo conclui que a melhor abordagem é **híbrida**. A aplicação deve fornecer validações e controlar os processos de negócio, enquanto o Banco de Dados deve proteger as regras fundamentais que garantem a integridade dos dados.
...
https://github.com/macedopvh/introducaoabancodedados
